...
#define SEF_VERSION "0.0.1"
#define SEF_TAB_STOP 8
#define SEF_QUIT_TIMES 3
...
int editorRowCxToRx(erow *row, int cx) {
  ...
  for (j = 0; j < cx; j++) {
    if (row->chars[j] == '\t')
      rx += (SEF_TAB_STOP - 1) - (rx % SEF_TAB_STOP);
    rx++;
  }
  return rx;
}
...
int editorRowRxToCx(erow *row, int rx) {
  ...
  for (cx = 0; cx < row->size; cx++) {
    if (row->chars[cx] == '\t')
      cur_rx += (SEF_TAB_STOP - 1) - (cur_rx % SEF_TAB_STOP);
    cur_rx++;
    if (cur_rx > rx)
      return cx;
  }
...
void editorUpdateRow(erow *row) {
  ...
  free(row->render);
  row->render = malloc(row->size + tabs * (SEF_TAB_STOP - 1) + 1);

  int idx = 0;
  for (j = 0; j < row->size; j++) {
    if (row->chars[j] == '\t') {
      row->render[idx++] = ' ';
      while (idx % SEF_TAB_STOP != 0)
        row->render[idx++] = ' ';
    } else {
    ...
    if (filerow >= E.numrows) {
      if (E.numrows == 0 && y == E.screenrows / 3) {
        ...
        int welcomelen = snprintf(welcome, sizeof(welcome),
                                  "sef editor -- version %s", SEF_VERSION);
